...
import org.example.delivery.domain.review.exception.ReviewBadRequestException;
import org.example.delivery.domain.review.exception.ReviewForbiddenException;
import org.example.delivery.domain.review.exception.ReviewNotFoundException;
...
public class ReviewService {
    ...
    public ReviewResponseDto createReview(Long userId,Long orderId,int rating,String content){
        Order order = orderRepository.findById(orderId).
                orElseThrow(() -> new ReviewNotFoundException( "해당 주문을 찾을 수 없습니다."));

        if (!order.getUser().getId().equals(userId)) {
            throw new ReviewForbiddenException( "주문자와 작성자가 다릅니다.");
        }

        if (!order.getStatus().equals(Order.Status.DELIVERED)){
            throw new ReviewBadRequestException("해당 주문엔 아직 리뷰를 달 수 없습니다.");

        }
        if(reviewRepository.existsByOrderId(orderId)) {
            throw new ReviewBadRequestException("이미 리뷰가 존재합니다.");
        }
    ...
    public void deleteReview(Long userId,Long reviewId){
        ...
        if (!review.getUser().getId().equals(userId)) {
            throw new ReviewForbiddenException( "본인이 작성한 리뷰만 삭제할 수 있습니다.");
        }
